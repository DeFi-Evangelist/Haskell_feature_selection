...
maxtri :: ((Float, Float, Float), [(a, Float)]) ->
...
mrmri :: ((Float, Float, Float), [(a, Float)]) -> [(a, a, Float)] -> 
...
mrmrn3 :: ((Float, Float, Float), [(a, Float)]) 
...
mutifl :: Ord a => ([a] -> [a] -> Float) -> [[a]] -> [(a, a, Float)]
mutifl f dtlst = [(i, k, (f (dtlst!!fromIntegral(i)) (dtlst!!fromIntegral(k)))) | i <- [0..n], k <- [0..n], i<k]
	where n = length dtlst

mutiol :: Ord a => ([a] -> [a] -> Float) -> [[a]] -> [a] -> a -> [(a, Float)]
mutiol f (x:xs) outs n = [(n, f x outs):(mutiol f xs outs (n+1))]
mutiol _ [] _ _ = []

mrmrdiscr :: Ord a => ([a] -> [a] -> Float) -> [[a]] -> [(a, Float)]
mrmrdiscr f dtlst outs = mrmrn3 ((z, z, z), setf) lf oxoxs
	where 
